...
type BoardButtonHandlerProps = { 
    action: string, 
    canPass?: undefined | boolean, 
    hands?: undefined | number[], 
    partners?: undefined | string[], 
    canDiscard: boolean,
    submitDiscard: () => void,
}

const BoardButtonHandler = ({ action, canPass, hands, partners, canDiscard, submitDiscard }: BoardButtonHandlerProps) => {
    ...
    useEffect(() => {
        switch (action) {
            ...
            case 'drawPreverTalon': setButtons([{name:'Pass Cards', callback: callbacks.passPreverTalon}, {name:'Keep Cards', callback: callbacks.drawPreverTalon}]); break;
            ...
            case 'discard': setButtons([{name:'Discard', callback: submitDiscard, disabled: !canDiscard}]); break;
            default: setButtons([]);
        }
    }, [action, canDiscard]);
    ...
    return (
        <div className="w-full h-full flex items-center justify-center pointer-events-auto gap-4">
            {buttons.map((button: ButtonArray, idx: number) => (
                <BoardButton key={idx} title={button.name} onClick={button.callback} disabled={button.disabled ?? false} />
            ))}
        </div>
    )
}
