...
function updateDateAndDirectory() {
    ...
    if (day != previousDay) { // new day new folder
        ...
        fs.mkdir(logsDirectory, { recursive: true }, (dirErr: Error) => {
            if (dirErr) {
                ...
                fs.writeFile(logFileName, '', { flag: 'a+' }, (fileErr: Error) => {
                    if (fileErr) {
...
export const SERVER = {
    ...
    */
    ...
    functionCall: (name: string, ...parameters: { name: string, value: string }[] ) => {
