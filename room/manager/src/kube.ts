...
export class Kube {
  ...
  async deployRoomToK8s(roomId: string): Promise<void> {
    ...
    for (const resource of resources) {
      if (resource.kind === "Deployment") {
        await this.#k8sApiApps.createNamespacedDeployment(
          Kube.NAMESPACE,
          resource
        );
      } else if (resource.kind === "Service") {
        await this.#k8sApiCore.createNamespacedService(
          Kube.NAMESPACE,
          resource
        );
      }
  ...
  async getRoom(roomId: string) {
    return await this.#k8sApiApps.readNamespacedDeployment(
      roomId,
      Kube.NAMESPACE
    );
  }
